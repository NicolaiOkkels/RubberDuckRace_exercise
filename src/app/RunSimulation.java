...
public class RunSimulation {
    ...
    public void run(int n){
        ...
        for (int i = 1; 1 <= (n-i) * (n-i); i++) {
            ...
            for (int j = 0; j < (n-i) * (n-i); j++) {
                ...
                while (isFetched){
                    int randomTemp = randomNumber(i,n); //Queue number to add duck to

                    isAddedToNewQueue = emptyQueuesTemp.get(randomTemp).addToQueue(duck,n-i); //add duck to next time step queues

                    if(isAddedToNewQueue){ //reset boolean after a duck is added to a new queue
                        isFetched = false;
                    }
                }
            }
    ...
    public List populateStartingQueues(int n){
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                tempQueue.get(i).addToQueue(new Duck(),n);
            }
        }
        return tempQueue;
    }
